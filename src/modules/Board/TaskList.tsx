...
export function TaskList({ list }: { list: List }) {
  const { data: Tasks, isLoading } = api.board.getTasks.useQuery({ listId: list.id || "" }, { enabled: Boolean(list.id), retry: false });
    ...
    <div className="h-full rounded-xl border-2" key={`main:${list.name}`}>
      ...
      >
        <SortableContext items={Tasks || []} strategy={verticalListSortingStrategy}>
          <div
            className="listScrollbar space-y-4 max-h-[75vh] overflow-y-scroll pb-2"
            ref={setNodeRef}
          >
            {Tasks?.length !== 0 ? (
              Tasks?.map((task) => (
      ...
      // bg={LIST_BG_COLOR}
      >
        <input className="w-full rounded-xl px-5 border-gray-200  bg-white " type="text" placeholder="add to list" />
