...
function RenameWorkspaceSection({ workspace, setIsOpen }: { workspace: Workspace, setIsOpen: Dispatch<SetStateAction<boolean>> }) {
    ...
    return (
        <div className="mt-2">
            <p className="font-semibold text-md text-neutral-500 dark:text-white">Rename Workspace</p>

            <Formik
                initialValues={{ name: workspace.name, workspaceId: workspace.id, }}
                validationSchema={toFormikValidationSchema(RenameWorkspaceValidationSchema)}
                onSubmit={
                    (values) => {
                        console.log(values);
                ...
                <Form>
                    <Field name="name">
                        {({ field, form, meta }: FieldProps) => (
                            <>
                                <label htmlFor="name" className="block mt-3 mb-2 text-sm font-medium text-neutral-500 dark:text-white">Workspace Name</label>
                                <div className="flex flex-row items-center justify-center gap-2">
                                    ...
                                    <PrimaryButton
                                        ...
                                        disabled={field.value === workspace.name || Object.keys(form.errors).length !== 0}
                                        type="submit"
