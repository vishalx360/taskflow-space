...
const OTP_EMAIL = async ({
  ...
  const filepath = path.join(
    process.cwd(),
    "src/utils/email-templates/otp-email.html"
  );
...
const BASIC_EMAIL = async ({
  ...
  const filepath = path.join(
    process.cwd(),
    "src/utils/email-templates/basic-email.html"
  );
  const rawHTML = await readFile(filepath, "utf8");
